...
class Solver(object):
    ...
    def __init__(self, celebA_loader, rafd_loader, config):
        ...
        # Sample settings
        self.sample_attributes = config.sample_attributes
    ...
    def sample(self):
        ...
        for att in self.sample_attributes:
            for pos in ['_pos', '_neg']:
                att_path = os.path.join(self.result_path, att+pos)
        ...
        att_counts = [0,0] * len(self.sample_attributes)
        for i, (real_x, org_c) in enumerate(data_loader):
            real_x = self.to_var(real_x, volatile=True)
            ...
            selected_attrs = data_loader.dataset.selected_attrs
            att_ind = [selected_attrs.index(att) for att in self.sample_attributes]

            # Start translations
            fake_image_list = [real_x]
            for j, att_i in enumerate(att_ind):
                target_c = target_c_list[att_i]
                fake_image = self.G(real_x, target_c)
                pos_ind = (target_c[:, att_i].data.cpu().numpy() == 1)
                for img_i in range(len(fake_image)):
                    if pos_ind[img_i]:
                        save_path = os.path.join(self.result_path, selected_attrs[att_i] + '_pos', '{:06d}.png'.format(att_counts[2*j]))
                        att_counts[2*j] += 1
                    else:
                        save_path = os.path.join(self.result_path, selected_attrs[att_i] + '_neg', '{:06d}.png'.format(att_counts[2*j+1]))
                        att_counts[2*j+1] += 1
                    save_image(self.denorm(fake_image[img_i:img_i+1].data), save_path, nrow=1, padding=0)
                    print(att_counts)
    ...
    def inference(self):
        ...
        transform = transforms.Compose([
            ...
            transforms.Scale(256, interpolation=Image.ANTIALIAS),
            transforms.ToTensor(),
            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])

        real_x = self.to_var(transform(Image.open(self.input_image)).unsqueeze(0), volatile=True)
        org_c  = torch.FloatTensor(np.array([self.input_label]))
        ...
        base_name, ext = os.path.splitext(self.input_image)
        save_path = '{}_fake.png'.format(base_name)
        save_image(self.denorm(fake_images.data), save_path, nrow=1, padding=0)
        print('Translated test images and saved into "{}"..!'.format(save_path))
